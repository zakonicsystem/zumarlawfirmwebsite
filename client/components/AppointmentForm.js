...
export default function AppointmentForm({ branches = [], services = [], content = {} }) {
  ...
  return (
    <form onSubmit={submit} className="w-full min-w-0 rounded-3xl border border-primary/10 bg-white p-5 shadow-xl shadow-primary/5 sm:rounded-[2rem] sm:p-9">
      <h2 className="text-2xl font-black text-primary sm:text-3xl">{content.formTitle || "Appointment Request"}</h2>
      ...
      <button className="mt-5 min-h-12 w-full rounded-full bg-primary px-6 text-sm font-black text-white sm:w-auto">{content.submitLabel || "Submit Appointment"}</button>
    </form>
  );
}
